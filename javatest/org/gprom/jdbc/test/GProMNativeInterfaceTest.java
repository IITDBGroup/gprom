...
public class GProMNativeInterfaceTest {
	...
	//	w.setupFromOptions(new String[] { "-log", "-loglevel", "4", "-backend", "oracle", "-user", "tpch", "-passwd", "IaDdpdr"});
		...
		log.error("has log.level " + w.optionExists("log.level"));
		log.error("has log.active " + w.optionExists("log.active"));
		log.error("has x " + w.optionExists("x"));
		
//		res = w.gpromRewriteQuery("PROVENANCE OF (SELECT * FROM r);");
//		log.error(res == null ? "NULL" : res);
		log.error("has x " + w.optionExists("x"));
		
		log.error("type of log.level: " + w.typeOfOption("log.level"));
		log.error("type of log.activate: " + w.typeOfOption("log.active"));
		log.error("type of connection.port: " + w.typeOfOption("connection.port"));
