...
void analyzeQueryBlockStmt(Node *stmt, List *parentFroms) {
	...
	case T_QueryBlock:
		DEBUG_LOG(
				"\n ###########################\n \t analyzeQueryBolckStmt() IN T_QueryBlock: \n ###########################\n");
	...
	case T_ProvenanceStmt:
		...
		DEBUG_LOG(
				"\n ###########################\n \t analyzeQueryBolckStmt() IN T_ProvenanceStmt: \n ###########################\n");
	...
	case T_List:
		DEBUG_LOG(
				"\n ###########################\n \t analyzeQueryBolckStmt() IN T_List: \n ###########################\n");
	...
	case T_CreateTable:
		DEBUG_LOG(
				"\n ###########################\n \t analyzeQueryBolckStmt() IN T_CreateTable:\n ###########################\n");
...
static void analyzeStmtList(List *l, List *parentFroms) {
	...
	{
		DEBUG_LOG(
				"\n#################################\n \t analyzeStmeList in iteration %d\n #################################\n ",
				++index);
		analyzeQueryBlockStmt(n, parentFroms);

		DEBUG_LOG(
				"\n#################################\n \t END ENDanalyzeStmeList in iteration %d END END\n #################################\n ",
				index--);
...
static void analyzeQueryBlock(QueryBlock *qb, List *parentFroms) {
	...
	FOREACH(FromItem,f,qb->fromClause)
	{
		...
		case T_FromTableRef: {
			...
			if (!tableExists && viewExists) {
				...
				view = s->data;
				Node *n1 = getHeadOfListP(
						(List*) parseFromString((char*) view));
				FromItem *f1 = createFromSubquery(newName, NIL, (Node*) n1);

				DUMMY_LC(f)->data.ptr_value = f1;
			}
			if (!tableExists && !viewExists)
				THROW(SEVERITY_RECOVERABLE, "table %s does not exist",
						tr->tableId);
		}
			break;
		default:
	...
	DEBUG_LOG("Found the following from tables: <%s>",
			nodeToString(qb->fromClause));
	...
	INFO_LOG("Expanded select clause is: <%s>",
			nodeToString(expandedSelectClause));
	...
	DEBUG_LOG("Have the following attribute references: <%s>",
			nodeToString(attrRefs));
	...
	if (f->provInfo) {
			...
			FOREACH(char,name,fp->userProvAttrs)
			{
				if (!searchListString(f->attrNames, name)) {
					if (strcmp(name, "ROWID") == 0
							|| f->type == T_FromTableRef) {
						f->attrNames = deepCopyStringList(f->attrNames);
						f->dataTypes = copyObject(f->dataTypes);
						f->attrNames = appendToTailOfList(f->attrNames,
								strdup("ROWID"));
						f->dataTypes = appendToTailOfListInt(f->dataTypes,
								DT_LONG);
					} else
						FATAL_LOG("did not find provenance attr %s in from "
								"item attrs %s", name,
								stringListToString(f->attrNames));
				}
			}
		}
		...
		if (fp->provProperties) {
			...
			if (getStringProvProperty(fp, PROV_PROP_TIP_ATTR)) {
				int pos = listPosString(f->attrNames,
						STRING_VALUE(
								getStringProvProperty(fp, PROV_PROP_TIP_ATTR)));
				DEBUG_LOG("TIP attribute %s at position %u",
						STRING_VALUE(getStringProvProperty(fp, PROV_PROP_TIP_ATTR)),
						pos);
				f->attrNames = deepCopyStringList(f->attrNames);
				f->dataTypes = copyObject(f->dataTypes);
			...
			if (getStringProvProperty(fp, PROV_PROP_V_TABLE)) {
				int pos = listPosString(f->attrNames,
						STRING_VALUE(
								getStringProvProperty(fp, PROV_PROP_V_TABLE)));
				DEBUG_LOG("VTABLE attribute %s at position %u",
						STRING_VALUE(getStringProvProperty(fp, PROV_PROP_V_TABLE)),
						pos);
				f->attrNames = deepCopyStringList(f->attrNames);
				f->dataTypes = copyObject(f->dataTypes);
				f->attrNames = removeListElemAtPos(f->attrNames, pos);
		...
		if (fp->baserel == FALSE && fp->intermediateProv == FALSE
				&& fp->userProvAttrs != NIL) {
			INFO_LOG(
					"from clause item HAS PROVENANCE activated - remove provenance attributes from schema for analysis");
...
static void analyzeFunctionCall(QueryBlock *qb) {
	...
	DEBUG_LOG("Have the following function calls: <%s>",
			nodeToString(functionCallList));
...
static void analyzeJoinCondAttrRefs(List *fromClause, List *parentFroms) {
		...
		if (isA(cur, FromJoinExpr)) {
			...
			FOREACH(AttributeReference,a,aRefs)
			{
				...
				if (LIST_LENGTH(nameParts) == 1) {
					...
					newFroms = appendToTailOfList(newFroms,
							LIST_MAKE(j->left, j->right));
				...
				// is R.A, search for table in subtree
					...
					// is in left subtree
						...
						isFound = findQualifiedAttrRefInFrom(nameParts, a,
								newFroms);
	...
	DEBUG_NODE_BEATIFY_LOG("finished adapting attr refs in join conds:",
			fromClause);
}
...
static int findAttrInFromItem(FromItem *fromItem, AttributeReference *attr) {
	...
	if (strpeq(attr->name,"ROWID") && fromItem->type == T_FromTableRef) {
		...
		fromItem->attrNames = appendToTailOfList(fromItem->attrNames,
				strdup("ROWID"));
		fromItem->dataTypes = appendToTailOfListInt(fromItem->dataTypes,
				DT_LONG);
	}
...
static boolean findQualifiedAttrRefInFrom(List *nameParts,
	...
	FOREACH(List,fromItems,fromClauses)
	{
		...
		FOREACH(FromItem, f, fromItems)
		{
			...
			if (foundF != NULL) {
				if (foundFrom) {
					FATAL_LOG(
							"from clause item name <%s> appears more than once",
							tabName);
					return FALSE;
				} else {
					fromItem = f;
	...
	if (!foundAttr) {
		FATAL_LOG("did not find any from clause item from attribute <%s>",
				attrName);
		return FALSE;
	}
...
static void analyzeLimitAndOffset(QueryBlock *qb) {
	...
	if (attrRefs != NIL) {
		THROW(SEVERITY_RECOVERABLE,
				...
				nodeToString(qb->limitClause), nodeToString(qb->offsetClause));
	}
		...
		FOREACH(char,name,f->attrList)
		{
			...
			if (!hasSetElem(attrNameSet, name))
				FATAL_LOG("INSERT mentions attribute <%s> that is not an "
						"attribute of table %s:<%s>", name, f->insertTableName,
...
//            int pos = 0;
			...
			FOREACH(AttributeDef,a,attrDefs)
			{
				...
				if (MAP_HAS_STRING_KEY(attrPos, a->attrName)) {
					...
					Node *def = getAttributeDefaultVal(
							...
							(char*) getNthOfListP(nameParts, 1), a->attrName); //TODO get schema
	...
	// is an INSERT INTO R (SELECT ...)
		...
		if (LIST_LENGTH(f->attrList) != attrNames->length) {
			...
			FOREACH(AttributeDef,a,attrDefs)
			{
				...
				if (MAP_HAS_STRING_KEY(attrPos, a->attrName)) {
					...
					val = (Node*) createFullAttrReference(strdup(a->attrName),
							0, INT_VALUE(MAP_GET_STRING(attrPos,a->attrName)),
			...
			wrap->fromClause = singleton(
					createFromSubquery(strdup("origInsertQuery"),
...
static void analyzeDelete(Delete *f) {
	...
	fakeTable = (FromTableRef*) createFromTableRef(strdup(f->deleteTableName),
			attrNames, strdup(f->deleteTableName), dataTypes);
...
static void analyzeUpdate(Update *f) {
	...
	fakeTable = (FromTableRef*) createFromTableRef(strdup(f->updateTableName),
			attrNames, strdup(f->updateTableName), dataTypes);
	fakeFrom = singleton(singleton(fakeTable));
...
splitAttrOnDot(char *dotName) {
	...
	TRACE_LOG("Split attribute reference <%s> into <%s>", dotName,
			stringListToString(result));
...
expandStarExpression(SelectItem *s, List *fromClause) {
			...
			FOREACH(char,attr,f->attrNames)
			{
				...
				if (!(f->type == T_FromTableRef && strcmp(attr, "ROWID") == 0)) {
					...
					newSelectItems = appendToTailOfList(newSelectItems,
							createSelectItem(strdup(attr), (Node*) newA));
				}
			}
		}
	}
	 ...
	 */
		...
		FOREACH(FromItem,f,leafItems)
		{
			if (strcmp(f->name, tabName) == 0) {
				if (found)
					FATAL_LOG(
							"Ambiguous from clause reference <%s> to from clause item <%s>",
							s->alias, tabName);
				else {
					FOREACH(char,attr,f->attrNames)
					{
						...
						if (!(f->type == T_FromTableRef
								&& strcmp(attr, "ROWID") == 0)) {
							newSelectItems = appendToTailOfList(newSelectItems,
									createSelectItem(strdup(attr),
											(Node*) createAttributeReference(
													CONCAT_STRINGS(f->name, ".",
															attr))));
						}
					}
				}
	...
	DEBUG_LOG("Expanded a star expression into <%s>",
			nodeToString(newSelectItems));
		...
		if (hasTimes) {
			FOREACH(KeyValue,sInfo,stmts)
			{
				if (!reenactOptionHasTimes((List*) sInfo->value)) {
					FATAL_NODE_BEATIFY_LOG(
							"AS OF should be specified for all statments to be reenacted or none!",
							q);
				}
			}
...
//        case PROV_INPUT_UPDATEPS: //NEW TYPE OF PROV_INPUT_UPDATEPS
	{
	...
	case PROV_INPUT_TEMPORAL_QUERY: {
		...
		if (tempDT == NULL) {
			FATAL_LOG(
					"sequenced temporal construct requires input to specify "
							"time attributes for FROM clause items using WITH TIME(...)");
		}

		q->selectClause = concatTwoLists(q->selectClause,
				LIST_MAKE(strdup(TBEGIN_NAME), strdup(TEND_NAME)));
		q->dts = concatTwoLists(q->dts,
				CONCAT_LISTS(singletonInt(*tempDT), singletonInt(*tempDT)));
	}
		break;
...
static boolean checkTemporalAttributesVisitor(Node *node, DataType **context) {
	...
	if (isFromItem(node)) {
		...
		if (p != NULL && p->userProvAttrs) {
			...
			leftDT = getNthOfListInt(f->dataTypes,
					listPosString(f->attrNames, leftName));
			rightDT = getNthOfListInt(f->dataTypes,
					listPosString(f->attrNames, rightName));
			tempD = leftDT;

			if (leftDT != rightDT)
				FATAL_LOG("attributes storing the interval endpoints have to "
						"have the same DTs (%s:%s != %s:%s):\n\n%s", leftName,
						DataTypeToString(leftDT), rightName,
						DataTypeToString(rightDT), beatify(nodeToString(node)));
		...
		// table references that are not part of a subquery which specifies temporal atttribute should specify temporal attributes
			...
			FATAL_LOG(
					"Table references in a sequenced temporal query you have "
...
static void analyzeCreateTable(CreateTable *c) {
	...
	if (tableExists)
		FATAL_LOG("trying to create table that already exists: %s",
				c->tableName);
