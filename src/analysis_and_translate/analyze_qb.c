...
static void analyzeFromSubquery(FromSubquery *sq, List *parentFroms) {
	...
	case T_QueryBlock: {
		...
		FOREACH(SelectItem,s,subQb->selectClause) {
			expectedAttrs = appendToTailOfList(expectedAttrs, s->alias);
		}
	}
		break;
	case T_SetQuery: {
...
splitAttrOnDot(char *dotName) {
	...
	TRACE_LOG("Split attribute reference <%s> into <%s>",
			dotName, stringListToString(result));
		...
		FOREACH(FromItem,f,fromClause) {
			if (strcmp(f->name, tabName) == 0) {
				if (found)
					...
					FOREACH(char,attr,f->attrNames) {
						newSelectItems =
								appendToTailOfList(newSelectItems,
