...
buildJoinTreeFromOperatorList(List *opList) {
	...
	FOREACH(QueryOperator, op, opList) {
		...
		List *attrNames = concatTwoLists(getAttrNames(oldRoot->schema),
				getAttrNames(op->schema));

		// create join operator
		root = (QueryOperator *) createJoinOp(JOIN_CROSS, NULL, inputs, NIL,
				attrNames);
...
translateFromClauseToOperatorList(List *fromClause) {
	...
	FOREACH(FromItem, from, fromClause) {
		...
		case T_FromSubquery:
			...
			break;
		default:
			FATAL_LOG("did not expect node <%s> in from list",
					nodeToString(from));
			break;
		}
...
createTableAccessOpFromFromTableRef(FromTableRef *ftr) {
	TableAccessOperator *ta = createTableAccessOp(ftr->tableId, NULL,
			ftr->from.name, NIL, ftr->from.attrNames, NIL); // TODO  get data types
	DEBUG_LOG("translated table access:\n%s\nINTO\n%s",
			nodeToString(ftr), nodeToString(ta));
...
translateFromJoinExpr(FromJoinExpr *fje) {
	...
	if (fje->joinCond == JOIN_COND_NATURAL) {
		...
		FOREACH(char,rA,rightAttrs) {
			...
			if (rPos != -1) {
				AttributeReference *lRef = createFullAttrReference(strdup(rA),
						0, lPos, 0);
				AttributeReference *rRef = createFullAttrReference(strdup(rA),
						1, rPos, 0);

				commonAttrs = appendToTailOfList(commonAttrs, rA);
				joinCond =
						AND_EXPRS((Node *) createOpExpr("=", LIST_MAKE(lRef,rRef)), joinCond);
			} else
		...
		"attrs <%s>, with left <%s> and right <%s>",
				stringListToString(commonAttrs), stringListToString(uniqueRightAttrs), stringListToString(leftAttrs), stringListToString(rightAttrs));
	...
	if (fje->joinCond == JOIN_COND_NATURAL) {
		...
		FOREACH(AttributeDef,a,input2->schema->attrDefs) {
			if (!searchListString(commonAttrs, a->attrName))
				projExpr = appendToTailOfList(projExpr,
						createFullAttrReference(strdup(a->attrName), 1, pos,
								0));
		}

		DEBUG_LOG("projection expressions for natural join: %s", projExpr);

		op = createProjectionOp(projExpr, (QueryOperator *) jo, NIL,
				fje->from.attrNames);
...
replaceNestedSubqueryWithAuxExpr(Node *node, int *state) {
	...
	if (isA(node, NestedSubquery)) {
		...
		int i = (*state)++;
		AttributeReference *attr = createAttributeReference(
				CONCAT_STRINGS("nesting_eval_", itoa(i)));
...
static boolean visitAttrRefToSetNewAttrPos(Node *n, List *state) {
	...
	if (isA(n, AttributeReference)) {
		...
		attrRef->attrPosition += getNthOfListInt(state,
				attrRef->fromClauseItem);
...
static QueryOperator *
translateHavingClause(Node *havingClause, QueryOperator *input,
		List *attrsOffsets) {
...
translateAggregation(QueryBlock *qb, QueryOperator *input, List *attrsOffsets) {
	...
	in = createProjectionOverNonAttrRefExprs(selectClause, havingClause,
			groupByClause, input, attrsOffsets);
...
static List *
getListOfNonAttrRefExprs(List *selectClause, Node *havingClause,
		List *groupByClause) {
