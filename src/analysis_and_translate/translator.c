...
translateSetQuery(SetQuery *sq, List *parents)
{
    List *inputs = appendToTailOfList(inputs, sq->lChild);
    inputs = appendToTailOfList(inputs, sq->rChild);
    ...
    if (sq->lChild->type == T_SetQuery)
        translateSetQuery((SetQuery *) sq->lChild, pa);
    else if (sq->lChild->type == T_QueryBlock)
        translateQueryBlock(((QueryBlock *) sq->lChild), pa);

    if (sq->rChild->type == T_SetQuery)
        translateSetQuery((SetQuery *) sq->rChild, pa);
    else if (sq->rChild->type == T_QueryBlock)
        translateQueryBlock(((QueryBlock *) sq->rChild), pa);
