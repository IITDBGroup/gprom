...
char* base64( const void* binaryData, int len, int *flen )
{
  ...
  INFO_LOG("Base64 encoding %d bytes of binary data\n", len);
  ...
  *flen = 4*(len + pad)/3 ; // (len+pad) IS divisible by 3
  ...
  INFO_LOG("%d %% 3 = %d, %d bytes pad, +1 byte NULL, flen=%d\n", len, lenMod3, pad, *flen);
  ...
  if( !base64String )
  {
    ...
    ERROR_LOG("I must stop because I could not get enough");
    // puts( "I must stop because I could not get enough" ) ;
    return 0;
  }
...
unsigned char* unbase64( const char* ascii, int len, int *flen )
{
  ...
  *flen = 3*(len/4) - pad;
  ...
  if( !bin )
  {
    ...
    // puts( "I must stop because I could not get enough" ) ;
    ERROR_LOG("ERROR: unbase64 could not allocate enough memory.");
    return 0;
  }
...
int base64integrity( const char *ascii, int len )
{
  ...
  for( i = 0 ; i < len - 2 ; i++ )
  {
    if( !isbase64ValidChr( ascii[i] ) )
    {
      ...
      INFO_LOG( "ERROR in base64integrity at chr %d [%c]. String is NOT valid base64.\n", i, ascii[i] ) ;
      return 0 ;
    }
  }
  ...
  if( ascii[i]=='=' )
  {
    ...
    if( ascii[i+1] != '=' )
    {
      ...
      INFO_LOG( "ERROR in base64integrity at chr %d.\n"
  ...
  else if( !isbase64ValidChr( ascii[i] ) )  // not = or valid base64
  {
    ...
    INFO_LOG( "ERROR in base64integrity at chr %d (2nd last chr). String is NOT valid base64.\n", i ) ;
    return 0 ;
  }
  ...
  if( ascii[i]!='=' && !isbase64ValidChr( ascii[i] ) )
  {
    ...
    INFO_LOG( "ERROR in base64integrity at chr %d (last chr). String is NOT valid base64.\n", i ) ;
    return 0 ;
  }
