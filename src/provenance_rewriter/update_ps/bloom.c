...
#include "provenance_rewriter/update_ps/murmurhash2.h"
...
#define BLOOM_ENTRY 10000000
#define BLOOM_ERROR 0.000001
Bloom *createBloomFilter()
{
  Bloom *bloom = makeNode(Bloom);
  bloom_init2(bloom, BLOOM_ENTRY, BLOOM_ERROR);
  nodeSetTag(bloom, T_Bloom);
  return bloom;
}
...
{
  if (bloom->ready == 0) {
    FATAL_LOG("bloom at %p not initialized!\n", (void *) bloom);
...
int bloom_init2(Bloom * bloom, unsigned int entries, double error)
{
  if (sizeof(unsigned long int) < 8) {
    FATAL_LOG("ERROR: libbloom will not function correctly");
...
int bloom_save(Bloom * bloom, char * filename)
{
  ...
  uint16_t size = sizeof(Bloom);
  out = write(fd, &size, sizeof(uint16_t));
  if (out != sizeof(uint16_t)) { goto save_error; }          // LCOV_EXCL_LINE

  out = write(fd, bloom, sizeof(Bloom));
  if (out != sizeof(Bloom)) { goto save_error; }      // LCOV_EXCL_LINE
...
int bloom_load(Bloom * bloom, char * filename)
{
  int rv = 0;
  ...
  if (size != sizeof(Bloom)) {
    rv = 7;
    goto load_error;
  }

  in = read(fd, bloom, sizeof(Bloom));
  if (in != sizeof(Bloom)) {
    rv = 8;
    goto load_error;
  }
...
int bloom_merge(Bloom * bloom_dest, Bloom * bloom_src)
{
