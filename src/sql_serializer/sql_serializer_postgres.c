...
//<<<<<<< HEAD
	...
	if (winR != NULL) {
		...
		List *inAttrs =
				(m->secondProj) ?
						firstProjs : getQueryOperatorAttrNames(m->fromRoot);
...
//                updateAttributeNamesSimple(expr, firstProjs);
// <<<<<<< HEAD
			UPDATE_ATTR_NAME((m->secondProj == NULL), wOp->partitionBy, fac,
					firstProjs);
			UPDATE_ATTR_NAME((m->secondProj == NULL), wOp->orderBy, fac,
					firstProjs);
			UPDATE_ATTR_NAME((m->secondProj == NULL), wOp->frameDef, fac,
					firstProjs);
		...
		DEBUG_LOG("window function translated, %s",
				stringListToString(windowFs));
	}
...
//        List *resultAttrs = NIL;
		...
		FOREACH(Node,a,p->projExprs)
		{
			...
			appendStringInfo(select, "%s%s", exprToSQL(a, NULL),
	...
	// else if window operator get the attributes from top-most window operator
		...
		DEBUG_LOG("window functions results as projection expressions %s",
				select->data);
	}
	// get aggregation result attributes
		...
		DEBUG_LOG("aggregation result as projection expressions %s",
				select->data);
	}
...
//        }
		...
		{
			FOREACH(char,name,attrs)
				inAttrs = appendToTailOfList(inAttrs,
						CONCAT_STRINGS("F", gprom_itoa(fromItem), "_",
								gprom_itoa(LIST_LENGTH(fac->fromAttrsList) - 1),
								".", name));
			fromItem++;
		}
		...
		DEBUG_LOG("FROM root attributes as projection expressions %s",
				select->data);
	}
...
// <<<<<<< HEAD
	...
	fac->fromAttrsList = appendToHeadOfList(fac->fromAttrsList,
			copyList(fac->fromAttrs));
	...
	if (HAS_STRING_PROP(t, PROP_USE_HISTORY_JOIN)) {
		...
		*attrOffset = 0;
		...
		if (HAS_STRING_PROP(t, PROP_TEMP_TNTAB)) {
			QueryOperator *inp = (QueryOperator*) LONG_VALUE(
					GET_STRING_PROP(t,PROP_TEMP_TNTAB));
			StringInfo tabName = makeStringInfo();
			QueryOperator *inpParent = (QueryOperator*) getHeadOfListP(
					inp->parents);
			api->createTempView(inp, tabName, inpParent, fac, api);
			appendStringInfo(from,
					"generate_series(1,(SELECT MAX(NUMOPEN) FROM (%s) F0)) F%u(n)",
...
//                    (*curFromItem)++);
			if (!getBoolOption(OPTION_PS_POST_TO_ORACLE)) {
				...
				appendStringInfo(from, "%s%s F%u_%u", t->tableName,
