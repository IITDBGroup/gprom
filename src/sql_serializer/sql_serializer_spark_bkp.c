...
char *
serializeOperatorModelSpark(Node *q)
{
    ...
    if (IS_OP(q))
    {
        ...
        appendStringInfoString(str, serializeQuerySpark((QueryOperator *) q));
        ...
        FOREACH(QueryOperator,o,(List *) q)
        {
            ...
            appendStringInfoString(str, serializeQuerySpark(o));
...
quoteAttributeNames (Node *node, void *context)
{
    ...
    if (isA(node, AttributeReference))
    {
        ...
        a->name = quoteIdentifierSpark(a->name);
    }
    if (isA(node, SelectItem))
    {
        SelectItem *a = (SelectItem *) node;
        a->alias = quoteIdentifierSpark(a->alias);
    }
    if (isA(node, AttributeDef))
    {
        AttributeDef *a = (AttributeDef *) node;
        a->attrName = quoteIdentifierSpark(a->attrName);
    }
...
char *
serializeQuerySpark(QueryOperator *q)
{
    StringInfo str;
    StringInfo viewDef;
    char *result;
    ...
    else
    {
        ...
        *fromAttrs = appendToTailOfList(*fromAttrs, attrNames);
        appendStringInfo(from, "(%s%s F%u)",
                quoteIdentifierSpark(t->tableName), asOf ? asOf : "",
                (*curFromItem)++);
    }
}
 ...
 */
char *
quoteIdentifierSpark (char *ident)
{
    int i = 0;
