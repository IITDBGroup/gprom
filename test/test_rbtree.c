...
test_tree_dt(Vector *DTs) {
    ...
    RBTRoot *rbtreeBC = makeRBT(RBT_ORDER_BY, TRUE);
    addToMap(rbtree->metadata, (Node *) createConstString("ORDER_BY_ASCs"), (Node *) copyObject(orderbyacs));
    ...
    for (int i = 0; i < totS; i++) {
        ...
        vecAppendNode(val, (Node *) bits);
        RBTInsert(rbtree, (Node *) key, (Node *) val);
        RBTInsert(rbtreeBC, (Node *) copyObject(key), (Node *) copyObject(val));
    }
